...
local_linelist <- get_international_linelist("Czech Republic") %>%
  tidyr::drop_na(date_confirm)

# Get WHO sit rep case counts ---------------------------------------------

total_cases <- TimeVaryingNCovR0::get_who_cases("Czechia", daily = TRUE)

# Join imported and local cases -------------------------------------------

cases <- TimeVaryingNCovR0::get_local_import_case_counts(total_cases, local_linelist) %>%
...
TimeVaryingNCovR0::analysis_pipeline(
  cases = cases,
  linelist = linelist,
  target_folder = file.path("inst/results/czechia", target_date),
  target_date = target_date,
  merge_actual_onsets = FALSE, samples = 10, save_plots = TRUE)
